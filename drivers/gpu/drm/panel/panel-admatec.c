...
static int admatec_panel_unprepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_disable(ARRAY_SIZE(admatec->supplies), admatec->supplies);
	if (ret < 0)
...
static int admatec_panel_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(admatec->supplies), admatec->supplies);
	if (ret < 0) {
...
static int admatec_panel_add(struct admatec_panel *admatec)
{
	...
	for (i = 0; i < ARRAY_SIZE(admatec->supplies); i++)
		admatec->supplies[i].supply = regulator_names[i];

	ret = devm_regulator_bulk_get(dev, ARRAY_SIZE(admatec->supplies),
				      admatec->supplies);
	if (ret < 0) {
